...
Then I ran it live against real Qwen output on Alibaba Cloud Model Studio, across 39 close tasks. Qwen3.7-Max was 100% accurate and the gate wrote zero wrong entries, a false-write rate of 0% with a Wilson 95% upper bound of 8.97%. The number that mattered most to me, though, was proving it is not zero by construction, so I ran a faster and weaker model, qwen-flash. It made four mistakes. The gate caught the cross-class ones live, including a cost-of-goods entry the model tried to post to accounts receivable and revenue, and two within-class account errors slipped through as a measured 5.41% false-write rate. That residual is the honest boundary of a class-level policy: it enforces the account class and the amount, not the choice between two accounts of the same class, and closing that gap by escalating ambiguous choices is the next step. A production claim would still need a prospective study against real closes with a gold standard, and I would rather say that plainly than quote a number I cannot defend.
